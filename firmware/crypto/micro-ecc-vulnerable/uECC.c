...
static void infinite_led_blink_green(void);
#endif
...
uECC_VLI_API uECC_word_t uECC_vli_testBit(const uECC_word_t *vli,
                                          bitcount_t bit) {
  int idx = bit >> uECC_WORD_BITS_SHIFT;                
...
static void EccPoint_double(uECC_word_t *result, const uECC_word_t *p, uECC_Curve curve) {
  ...
  uECC_word_t temp1[uECC_MAX_WORDS];
  uECC_word_t temp2[uECC_MAX_WORDS];
  uECC_word_t temp3[uECC_MAX_WORDS];
  const static uECC_word_t a[] = { 
    BYTES_TO_WORDS_8(FC, FF, FF, FF, FF, FF, FF, FF),
...
static void EccPoint_add(uECC_word_t *result, const uECC_word_t *p,
                          const uECC_word_t *q, uECC_Curve curve) {
  uECC_word_t p_x[uECC_MAX_WORDS];
  uECC_word_t p_y[uECC_MAX_WORDS];
 ...
 // Check if point p is the zero point
  if (uECC_vli_isZero(p, curve->num_words*2)) {
      // Return point q
  ...
  // Check if point q is the zero point
  ...
  // Check if points p and q are the same
  if (uECC_vli_equal(p, q, curve->num_words*2)) {
      // Points are the same, perform point doubling
      EccPoint_double(result, p, curve);
      return;
  }

  // Copy coordinates of points p and q
  uECC_vli_set(p_x, p, curve->num_words);
  uECC_vli_set(p_y, p + curve->num_words, curve->num_words);
...
static void EccPoint_mult(uECC_word_t *result, const uECC_word_t *point,
                          ...
                          uECC_Curve curve) {
  uECC_word_t R[uECC_MAX_WORDS*2];
